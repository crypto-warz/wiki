...
Trezor Suite включает несколько важных механизмов для проверки подлинности аппаратных кошельков, включая прошивку. Эти проверки производятся автоматически и не очевидны для пользователя. Кроме того, могут наблюдаться ложные срабатывания системы защиты.
...
Каждый бинарный файл прошивки имеет несколько свойств, включая версию прошивки и атрибут RevisionID. Эти свойства хранятся в Trezor Suite, а также на удаленном сервере (в репозитории [Data](https://github.com/trezor/data/)), что означает, что мы знаем обо всех выпущенных версиях прошивки (плюс их ревизии).

Ваше устройство Trezor передает эти свойства в приложение Trezor Suite, и они сверяются с базой данных версий, хранящейся в приложении. Если эти атрибуты совпадают, то ничего не происходит, и вы можете продолжать пользоваться Trezor и приложением Suite как обычно. Однако если они не совпадают, Trezor Suite отобразит следующее предупреждение:
...
В десктопном приложении Trezor Suite проверка выполняется только в том случае, если устройство обновлено до последней версии прошивки, предлагаемой приложением. Однако веб-приложение Trezor Suite выполняет проверку хэш-суммы для всех прошивок Trezor One версии ≥1.11, поскольку автоматически загружает бинарный файл нужной прошивки, и версии ≥2.6.0 для всех остальных устройств.
 
...
1. **Проверка соответствия**: Trezor Suite выдает устройству случайный криптографический вызов. Затем устройство вычисляет хэш-сумму прошивки, используя случайное число от Suite. Полученное значение сравнивается с хэш-суммой, вычисленной с использованием того же случайного числа из официального бинарного файла прошивки, хранящегося в Trezor Suite.
...
3. **Восстановление кошелька:** Trezor Suite восстанавливает кошелек из своей энтропии, которая была использована на шаге 1, и энтропию, предоставленную устройством, чтобы проверить, создаются ли точно такие же **XPUB** (расширенные открытые ключи), как и на аппаратном кошельке. **Этот процесс не подвергает риску ни один приватный ключ, поскольку каждый кошелек, созданный во время тестирования, отбрасывается и никогда не используется — он существует на устройстве лишь короткое время для проверки энтропии**.
...
Аппаратные кошельки Trezor Safe 3 и Safe 5 имеют дополнительный безопасный чип ([Secure Element](https://trezor.io/learn/a/secure-element-in-trezor-safe-devices)). Помимо обеспечения дополнительного уровня защиты от физических атак, этот чип играет важную роль в проверке подлинности устройства.
